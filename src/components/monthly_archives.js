    ...
    return (
        <TreeView 
            ...
            {data.monthlyArchivesByYear.group.sort((a, b) => b.year - a.year).map(year_node => (
                <TreeItem nodeId={year_node.year} label={year_node.year}>
                        {
                            year_node.nodes.sort((a, b) => a.context.month - b.context.month).map(node => (
                                <TreeItem nodeId={node.id} label={node.context.year + '/'+ node.context.month.toString().padStart(2,' ') + ' ('+node.context.count+')'} onLabelClick={ () => {handleClick(node)}}>
